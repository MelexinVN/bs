...
void RX433_Int()
{
	...
	{//если текущее состояние вывода - 1, прошедший интервал был 0
		if(cur_status == 1)																						
		{//если длительность между 200 и 400 мкс или импульсов преамбулы еще не было, 
	...
	if(preamble_count == PREAM_NUM_IMP)		//если приамбула принята полностью
	{
		...
		{			//если импульс в диапазоне
			...
			{		//если импульс короткий, записываем в массив 0, если длинный - 1
				bit_array[bit_counter] = (pulse_duration < DURATION) ? 0 : 1;  
				...
				if (bit_counter == SIZE_ARRAY)	//если приняты все байты
				{
					...
					{//сдвигаем значение влево и прибавляем текущее значение бита (в обратном порядке)
						dev_addr = (dev_addr << 1) + bit_array[7 - i]; 
					}
