...
{//Обработка приема радиомодуля
	...
	if(rx_flag == 1)				//если флаг приема поднят
	{
		...
		if (update_mode)					//если режим обновления прошивки
		{
			if (rx_buf[0] == 0x07)	//если адрес того модуля, который перепрошиваем
			{
				if (rx_buf[1] == 'W')	//если ответ готовности ждать загрузки
				{
					update_respond = 1;	//поднимаем флаг подтверждения запроса загрузки
				}
				if (rx_buf[1] == 'N')	//если ответ - запрос следующей строки
				{
					next_row = 1;				//поднимаем флаг отправки следующей строки
				}
			}
		}
		else
